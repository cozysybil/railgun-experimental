const artifacts = require("@railgun-community/circuit-artifacts");
 ...
 */
function matchG1Point(point1, point2) {
 ...
 * @returns points match
 */
function matchG2Point(point1, point2) {
 ...
 * @returns formatted vkey
 */
function formatVKeyMatcher(vkey) {
  ...
  return (i) => {
    ...
    if (!matchG1Point(iCast.alpha1, vkeySolidity.alpha1)) return false;

    // Check beta point
    if (!iCast.beta2) return false;
    if (typeof iCast.beta2 !== "object") return false;
    if (!matchG2Point(iCast.beta2, vkeySolidity.beta2)) return false;

    // Check beta point
    if (!iCast.gamma2) return false;
    if (typeof iCast.gamma2 !== "object") return false;
    if (!matchG2Point(iCast.gamma2, vkeySolidity.gamma2)) return false;

    // Check beta point
    if (!iCast.delta2) return false;
    if (typeof iCast.delta2 !== "object") return false;
    if (!matchG2Point(iCast.delta2, vkeySolidity.delta2)) return false;
    ...
    for (let index = 0; index < iCast.ic.length; index += 1) {
      if (!matchG1Point(iCast.ic[index], vkeySolidity.ic[index])) return false;
    }
 ...
 * @returns complete
 */
async function loadAvailableArtifacts(verifierContract) {
