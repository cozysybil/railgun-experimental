...
const { hash, randomBytes } = require("../global/crypto");
const {
  hexStringToArray,
  arrayToBigInt,
  bigIntToArray,
...
const { getKeys } = require("./artifacts");
 ...
 * @returns matcher
 */
function nullifiersMatcher(nullifiers) {
  return (contractNullifiers) => {
 ...
 * @returns matcher
 */
function ciphertextMatcher(ciphertextVector) {
 ...
 * @returns matcher
 */
function shieldCiphertextMatcher(shieldCiphertext) {
  // Return constructed matcher function
  return (contractShieldCiphertext) => {
    // If lengths don't match return false
    ...
    const shieldCiphertextMatched = contractShieldCiphertext.map(
      (ciphertext, shieldCiphertextIndex) => {
        // Check ciphertext words match
 ...
 * @returns matcher
 */
function commitmentPreimageMatcher(commitmentPreimages) {
  return (contractPreimages) => {
    // Loop through each preimage and check if they match
    const preimagesMatched = contractPreimages.map((preimage, index) => {
      if (
        preimage.npk !== arrayToHexString(commitmentPreimages[index].npk, true)
      )
        return false;
      if (
        preimage.token.tokenType !== commitmentPreimages[index].token.tokenType
      )
        return false;
      if (
        ...
        return false;
      if (
        preimage.token.tokenSubID.toBigInt() !==
        commitmentPreimages[index].token.tokenSubID
      )
        return false;
      if (preimage.value.toBigInt() !== commitmentPreimages[index].value)
        return false;
      return true;
    });
 ...
 * @returns matcher
 */
function tokenDataMatcher(tokenData) {
  return (contractTokenData) => {
    ...
    if (contractTokenData.tokenSubID.toBigInt() !== tokenData.tokenSubID)
      return false;
    return true;
  };
}
 ...
 */
function padWithDummyNotes(originalBundle, outputsLength) {
...
async function formatCircuitInputs(
  ...
  return {
    ...
    publicKey: publicKey.map(arrayToBigInt),
    signature: signature.map(arrayToBigInt),
...
async function dummyTransact(
  ...
  console.log("senderViewingPrivateKey")
  console.debug(senderViewingPrivateKey)
  ...
  console.log("commitmentCiphertext")
  console.debug(commitmentCiphertext)
