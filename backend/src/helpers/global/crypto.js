...
const { arrayToBigInt, bigIntToArray, arrayToByteLength } = require("./bytes");
...
const ed25519 = {
   ...
   * @param endian - what endian to use 'be' | 'le'
   * @returns adjusted bytes
   */
  adjustBytes25519(bytes, endian) {
   ...
   */
  privateKeyToPrivateScalar(privateKey) {
...
const edBabyJubJub = {
   ...
   * @returns signature
   */
  async signPoseidon(key, message) {
    const eddsaBuild = await eddsaPromise;
...
module.exports = { randomBytes, hash, aes, ed25519, edBabyJubJub };
