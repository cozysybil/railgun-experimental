...
task("deploy:test", "Creates test environment deployment").setAction(
  async function (taskArguments, hre) {
    ...
    const TestERC202 = await ethers.getContractFactory("TestERC20");
    ...
    const RailgunSmartWallet = await ethers.getContractFactory(
    ...
    await logVerify("Delegator", delegator, [
    ...
    await logVerify("Proxy Admin", proxyAdmin, [
      (await ethers.getSigners())[0].address,
    ]);
    ...
    await (
      await proxyAdmin.upgrade(
        ...
        treasuryImplementation.address
      )
    ).wait();
    ...
    console.log(
      `\nGiving full governance permissions to ${
        (await ethers.getSigners())[0].address
      }`
    );
    ...
    const testERC202 = await TestERC202.deploy();
    await logVerify("Test ERC202", testERC202, []);

    const simpleSwap = await SimpleSwap.deploy();
    await logVerify("Test ERC202", simpleSwap, []);
